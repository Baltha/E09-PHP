...
// Log dans log_errors.php

ini_set('log_errors', true);
ini_set('error_log', dirname(__FILE__).'/ipn_errors.log');

// initiation du php listener (écouteur d'evenement paypal)
include('lib/ipn/ipnlistener.php');

$listener = new IpnListener();

// Utilisation de Sandbox Paypal ou non
$listener->use_sandbox = true;

// Essayer d'écouter le post Ipn
try {
...
if ($verified) {
    ...
    // 1. Vérification du paiement complété
    ...
    // 2. Vérification de l'adresse du vendeur.
    if ($f3->get('POST.receiver_email') != 'paypal@skeelbox.com') {
        $errmsg .= "'receiver_email' does not match: ";
        $errmsg .= $f3->get('POST.receiver_email').'\n';
    }
  
    // 3. Vérification si la transaction a déjà eu lieu
    // mysql_connect('localhost', 'root', '2ZMu3CNMHBZy2r7s') or exit(0);
    // mysql_select_db('speedaudit') or exit(0);
    ...
    // if ($exists) {
    //     $errmsg .= "'txn_id' a déjà été pris en compte: ".$_POST['txn_id']."\n";
    // }
    
    if (!empty($errmsg)) {
    
        $to = 'louis.serre@gmail.com';
        
    // Dans le cas ou une erreur IPN a eu lieu, envois d'un rapport aux admins
        $body = "IPN failed fraud checks: \n$errmsg\n\n";
        $body .= $listener->getTextReport();
        mail($to, 'IPN Fraud Warning', $body);
    ...
    //Récupération des variables customs
    

    // $result_event = $_POST['custom'];
    // list($name, $date, $society, $email, $url, $tel, $typeaudit, $idevent) =  explode('|', $result_event);
    
    
    // $date = str_replace("-"," à ",$date);
    // $date = str_replace("_"," ",$date);
    // $typeaudit = str_replace("_"," ",$typeaudit);
    // $typeaudit = str_replace("A","a",$typeaudit);
    // $typeaudit = str_replace("C","c",$typeaudit);
    // $email = str_replace( "-","@",$email);
    // $name = str_replace("_"," ",$name);

    $to = 'louis.serre@gmail.com';

    mail($to, 'VALID IPN', $listener->getTextReport());
  ...
  $to = 'louis.serre@gmail.com'  . ' , ';
    ...
    // Dans le cas ou une erreur IPN a eu lieu, envois d'un rapport aux admins
    mail($to, 'Invalid IPN', $listener->getTextReport());
}
